=== Block Aarambha ===

Contributors: aarambhathemes
Tags: custom-colors, custom-menu, custom-logo, editor-style, grid-layout, one-column, featured-images, full-site-editing, block-patterns, sticky-post, wide-blocks, block-styles, template-editing, full-width-template, blog, news, portfolio
Requires at least: 6.1
Tested up to: 6.1
Requires PHP: 7.2
Stable tag: 1.0.1
License: GPLv3 or later
License URI: https://www.gnu.org/licenses/gpl-3.0.en.html

== Description ==

Block Aarambha is a full site editing Blog WordPress Theme. It is a modern block-based theme distinctively developed with ease of use and neat design. This theme is particularly developed to help you to make your unique-looking website. As it is a block-based theme, you can alter every component of the site including page layouts, typography, colors, etc.

== Copyright ==

Block Aarambha WordPress Theme, Copyright (c) 2023, Aarambha Themes
Block Aarambha is distributed under the terms of the GNU GPL

Block Aarambha Theme bundles the following third-party resources:

Roboto Font
Copyright (c) 2012, Pablo Impallari, Rodrigo Fuenzalida, with Reserved Font Name Libre Baskerville(https://github.com/impallari/Libre-Baskerville)
License: Apache License, Version 2.0
Source: https://fonts.google.com/specimen/Roboto/about

Libre Baskerville Font
Copyright (c) 2012, Pablo Impallari, Rodrigo Fuenzalida, with Reserved Font Name Libre Baskerville(https://github.com/impallari/Libre-Baskerville)
License: SIL Open Font License, Version 1.1
Source: https://fonts.google.com/specimen/Libre+Baskerville/about

TGM Plugin Activation
Copyright (c) 2011, Thomas Griffin
License: GNU General Public License v2.0 - https://github.com/TGMPA/TGM-Plugin-Activation/blob/develop/LICENSE.md
Source: https://github.com/TGMPA/TGM-Plugin-Activation

Images for theme screenshot:

Logo,
License: CC0 license - https://creativecommons.org/about/cc0

== Image Used ==

License: pxhere provides images under CC0 1.0 Universal (CC0 1.0) 
license: https://creativecommons.org/publicdomain/zero/1.0/
Source:  https://pxhere.com/en/photo/144494
         https://pxhere.com/en/photo/716411
         https://pxhere.com/en/photo/1028432

All images from assets/images
hero.jpg
License: pxhere provides images under CC0 1.0 Universal (CC0 1.0) 
license: https://creativecommons.org/publicdomain/zero/1.0/
Source:  https://pxhere.com/en/photo/1400971